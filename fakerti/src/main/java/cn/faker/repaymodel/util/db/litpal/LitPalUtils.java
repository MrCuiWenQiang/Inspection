...
public class LitPalUtils {
    ...
    public static <T> List<T> selectPage(Class<T> t, int page, int size, String... conditions) {
        ...
        List<T> datas = DataSupport.where(conditions).offset(offset).limit(size).find(t);
        return datas;
    }
    ...
    public static <T> List<T> selectPage(Class<T> t, int page, int size, String column, String... conditions) {
        ...
        int offset = (page - 1) * size;
        List<T> datas = DataSupport.where(conditions).offset(offset).limit(size).order(column).find(t);
        return datas;
    }
...
/*    public static <T> List<T> selecActivitytWhere(Class<T> t, String... conditions) {
        List<T> list = DataSupport.select("id", "nId", "cadreName", "professional", "area", "arcNumber", "workPlace", "sex", "flag", "status", "cadreStatus", "spelling", "createTime", "editTime", "virson", "rfid", "cabId", "onFrameTime", "offFrameTime").where(conditions).find(t);
        return list;
    }
    ...
    public static <T> List<T> selectWhere(Class<T> t, String... conditions) {
        List<T> list = DataSupport.where(conditions).find(t);
        return list;
    }

    public static <T> List<T> selectWheres(String column, Class<T> t, String... conditions) {
        List<T> list = DataSupport.where(conditions).order(column).find(t);
        return list;
    }
    ...
    public static <T> int selectCount(Class<T> t, String... conditions) {
        try {
            return DataSupport.where(conditions).count(t);
        } catch (Exception e) {
            return 0;
        }
    }
    ...
    public static <T> int deleteData(Class<T> t, String... conditions) {
        return DataSupport.deleteAll(t, conditions);
    }
}
