...
// TODO: 2020/6/15 后缀名问题
    ...
    public static void get(@NonNull String url, HashMap<String, Object> params, @NonNull Callback callback) {
        if (params != null && params.size() > 0) {
        ...
        if (callback instanceof HttpResponseCallback){
            ((HttpResponseCallback)callback).setOnFailedAll(onFailedAll);
        }
        Request request = null;
        request = new Request.Builder().url(url).get().build();
        if (httpHelper == null) {
